...
const main = module.exports = async(linkOrId, options, rt = 3) => {
  ...
  const resp = {
    ...
    title: UTIL.parseText(info.title, '') ||
      UTIL.parseText(parsed.json.header.playlistHeaderRenderer.title, ''),
    ...
    description: info.description ? UTIL.parseText(info.description, '') :
      UTIL.parseText(parsed.json.header.playlistHeaderRenderer.descriptionText, ''),
